...
class AdministrativeCommunication(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        if employee:
            domain += ['|', ('user_id', '=', self.env.user.id), ('create_uid', '=', self.env.user.id)]
...
class AdministrativeCommunicationLine(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        # if center_manager:
        #     args += []
        return super(AdministrativeCommunicationLine, self).search(args, offset=offset, limit=limit, order=order,count=count)
